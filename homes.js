/**
 * NexTrade — Home Dashboard (FIXED: Scroll + Profile + Institutional Design)
 * ══════════════════════════════════════════════════════════════════════
 * CRITICAL FIXES:
 * 1. Added explicit container styles (fixes scroll lock)
 * 2. Uses AppState.get('profile') for username (fixes "Trader")
 * 3. Institutional hero card matching wallet.js design
...
const Home = (() => {
  ...
  async function render(element) {
    ...
    // 🔥 CRITICAL FIX: Set explicit container styles (prevents scroll lock)
    container.style.cssText = 'display: flex; flex-direction: column; height: 100%; overflow-y: auto; overflow-x: hidden; -webkit-overflow-scrolling: touch;';

    // Sync state from AppState
    if (window.AppState) {
      state.user = AppState.get('user');
      state.profile = AppState.get('profile'); // 🔥 CRITICAL: Get profile separately
      state.balances = AppState.get('balances') || { spot: 0, vault: 0 };
      state.holdings = AppState.get('holdings') || {};
    }

    showSkeleton();
    loadChartLibrary().catch(err => console.warn('[HOME] Chart.js load failed:', err));

    const cachedData = getCachedData();
    if (cachedData.hasCache) {
      console.log('[HOME] 📦 Rendering with cached data');
      state.marketData = cachedData.marketData;
      state.globalMetrics = cachedData.globalMetrics;
      buildUI();
    }
  ...
  function showSkeleton() {
    ...
    container.appendChild(createSkeletonHero());
    container.appendChild(createSkeletonActions());
    container.appendChild(createSkeletonPulse());
    container.appendChild(createSkeletonChart());
  }
  ...
  function createSkeletonHero() {
    ...
    skeleton.innerHTML = `
      <div class="skeleton-pulse" style="width:120px; height:12px; background:rgba(255,255,255,0.1); border-radius:4px; margin:0 auto 12px;"></div>
      <div class="skeleton-pulse" style="width:200px; height:40px; background:rgba(255,255,255,0.15); border-radius:8px; margin:0 auto 16px;"></div>
      <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:12px;">
        <div class="skeleton-pulse" style="height:60px; background:rgba(255,255,255,0.08); border-radius:8px;"></div>
        <div class="skeleton-pulse" style="height:60px; background:rgba(255,255,255,0.08); border-radius:8px;"></div>
        <div class="skeleton-pulse" style="height:60px; background:rgba(255,255,255,0.08); border-radius:8px;"></div>
      </div>
    `;
  ...
  function createSkeletonActions() {
    const skeleton = document.createElement('div');
    skeleton.style.cssText = 'display:grid; grid-template-columns:repeat(3,1fr); gap:10px; margin-bottom:20px;';
    
    for (let i = 0; i < 3; i++) {
      const box = document.createElement('div');
      box.className = 'skeleton-pulse';
      box.style.cssText = 'height:72px; background:var(--color-surface); border-radius:12px;';
  ...
  function createSkeletonPulse() {
    const skeleton = document.createElement('div');
    skeleton.className = 'skeleton-pulse';
    skeleton.style.cssText = 'height:180px; background:var(--color-surface); border-radius:12px; margin-bottom:24px;';
    return skeleton;
  }
  ...
  function createSkeletonChart() {
    const skeleton = document.createElement('div');
    skeleton.className = 'skeleton-pulse';
    skeleton.style.cssText = 'height:340px; background:var(--color-surface); border-radius:12px; margin-bottom:24px;';
    return skeleton;
  }
  ...
  function getCachedData() {
    ...
    if (window.AppState) {
      const appMarketData = AppState.get('marketData');
      if (appMarketData && Array.isArray(appMarketData) && appMarketData.length > 0) {
        cached.marketData = appMarketData;
        cached.hasCache = true;
      }
    }

    if (state.globalMetrics) {
      cached.globalMetrics = state.globalMetrics;
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (document.querySelector('script[src*="chart.js"]')) {
        ...
        setTimeout(() => {
          ...
          else reject(new Error('Chart.js timeout'));
        }, 5000);
        return;
      }
  ...
  async function loadAllDataInBackground() {
    console.log('[HOME] 🚀 Loading all data in background...');
    ...
    try {
      ...
      results.forEach((result, index) => {
        if (result.status === 'rejected') {
          const names = ['market', 'global', 'trending'];
          console.warn(`[HOME] ⚠️ ${names[index]} data failed:`, result.reason);
        }
      });
      ...
      console.error('[HOME] ❌ Background load error:', error);
    }
  }
  ...
  async function loadMarketData() {
    ...
    try {
      if (window.CacheManager && typeof CacheManager.getMarketData === 'function') {
        const result = await CacheManager.getMarketData();
        
        if (result.success && result.data && result.data.length > 0) {
          state.marketData = result.data;
          console.log(`[HOME] ✅ Loaded ${result.data.length} coins from CacheManager`);
          
          if (window.API && typeof API.ensureCoinLoaded === 'function') {
      ...
      if (window.API && typeof API.getMarketData === 'function') {
        const result = await API.getMarketData(50, 'usd', false);
        
        if (result.success && result.data && result.data.length > 0) {
          state.marketData = result.data;
          console.log(`[HOME] ✅ Loaded ${result.data.length} coins from API`);
        }
      }
  ...
  async function loadGlobalMetrics() {
    ...
    try {
      ...
      if (data && data.data) {
        ...
        console.log('[HOME] ✅ Global metrics loaded');
      }
      
      if (window.API && typeof API.getFearGreedIndex === 'function') {
  ...
  // 🔥 CRITICAL FIX: Use First Name from Full Name
  function formatName() {
    if (!state.profile) return 'Trader';
    
    // 1. Try to get First Name from Full Name
    if (state.profile.full_name) {
      return state.profile.full_name.split(' ')[0]; // Returns "Momo" from "Momo Smith"
    }
  ...
  function createHeader() {
    ...
    header.innerHTML = `
      <div style="font-size:14px; color:var(--color-text-secondary); font-weight:500;">
        ${getGreeting()},
      </div>
      <div style="font-size:24px; font-weight:800; color:var(--color-text-primary); letter-spacing:-0.5px;">
        ${userName}
      </div>
    `;
    return header;
  ...
  function createHeroCard() {
    ...
    // 🔥 INSTITUTIONAL DESIGN: Match wallet.js hero card
    card.style.cssText = `
      position: relative;
      overflow: hidden;
      border-radius: 16px;
  ...
  function createActionTile(label, icon, color, actionName) {
  return `
    <button data-action="${actionName}" style="
  ...
  function createMarketPulse() {
    ...
    if (!state.globalMetrics) {
      section.innerHTML = `
        ...
        <div class="card skeleton-pulse" style="height:180px; background:var(--color-surface); border-radius:12px;"></div>
      `;
      return section;
    }
    ...
    section.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
        <h3 style="font-size:15px; font-weight:700; color:var(--color-text-primary); margin:0;">
          <i class="fas fa-globe" style="margin-right:8px; color:#8b5cf6;"></i>Market Pulse
        </h3>
      </div>
  ...
  function createSmartAlerts() {
    const section = document.createElement('div');
    section.style.cssText = 'margin-top: 24px; padding-bottom: 40px;';
  ...
  async function initChart() {
    ...
    if (!canvas) {
      console.warn('[HOME] ⚠️ Chart canvas not found');
      return;
    }

    if (typeof Chart === 'undefined') {
      console.warn('[HOME] ⚠️ Chart.js not loaded yet');
      return;
    }
  ...
  function startLiveUpdates() {
    ...
    pulseUpdateInterval = setInterval(async () => {
      if (container && document.body.contains(container)) {
        ...
        if (pulseCard && state.globalMetrics) {
