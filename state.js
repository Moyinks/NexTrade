 ...
 *
 * FIX (balance-integrity): Added `balanceSyncStatus` to AppState.
 * Allowed values: 'syncing' | 'ready' | 'error'
 * Default is 'syncing' so any render that fires before cloud hydration is
 * complete is gated. apps.js sets it to 'ready' after all data sources
 * resolve, and home.js gates the hero card on this flag.
 * loadFromStorage() deliberately does NOT persist or restore this field —
 * it must always start as 'syncing' on every page load so stale cached
 * balances are never presented as current truth.
 */
...
(function () {
  ...
  const getDefaults = () => ({
    ...
    // Balance readiness — always starts as 'syncing' on every page load.
    // Never persisted to localStorage. apps.js sets this to 'ready' after
    // all cloud data sources (profile, investments, market) have resolved.
    // home.js gates the hero card balance display on this flag.
    balanceSyncStatus: 'syncing', // 'syncing' | 'ready' | 'error'
    ...
    try {
      if (_state.user) {
        const data = {
          ...
          ui:          _state.ui
          // balanceSyncStatus is intentionally NOT persisted.
          // It must always reset to 'syncing' on the next page load.
        };
  ...
  const loadFromStorage = () => {
    try {
      ...
      // balanceSyncStatus is NOT read from storage — it stays 'syncing'
      // so the UI never renders stale cached balances as authoritative.
